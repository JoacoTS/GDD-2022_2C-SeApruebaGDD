...
create table SE_APRUEBA_GDD.BI_TIPO_DESCUENTO (
	...
	TIPO_DESCUENTO_MONTO decimal(18, 2),
...
create table SE_APRUEBA_GDD.BI_DESCUENTO (
    FECHA_ID decimal(19,0),
    MEDIO_PAGO_ID decimal(19,0),
    CANAL_VENTA_ID decimal(19, 0),
...
begin
	insert into SE_APRUEBA_GDD.BI_TIPO_DESCUENTO(TIPO_DESCUENTO_ID, TIPO_DESCUENTO_DESCRIPCION) values ('Otros')
  insert into SE_APRUEBA_GDD.BI_TIPO_DESCUENTO(TIPO_DESCUENTO_ID, TIPO_DESCUENTO_DESCRIPCION) values ('Cupon')
  insert into SE_APRUEBA_GDD.BI_TIPO_DESCUENTO(TIPO_DESCUENTO_ID, TIPO_DESCUENTO_DESCRIPCION)
...
begin
	insert into SE_APRUEBA_GDD.BI_VENTA (FECHA_ID, CLIENTE_ID, RANGO_ID, MEDIO_PAGO_ID, CANAL_VENTA_ID, TIPO_ENVIO_ID, PROVINCIA_ID, PRODUCTO_ID, CATEGORIA_ID, DESCUENTO_VENTA_ID, CANTIDAD, PRECIO)
  select
    VENTA_ID,
    FECHA_ID,
    CLIENTE_ID,
...
create procedure SE_APRUEBA_GDD.BI_MIGRAR_DESCUENTO as
begin
	insert into SE_APRUEBA_GDD.BI_DESCUENTO (FECHA_ID,
    MEDIO_PAGO_ID,
    CANAL_VENTA_ID,
    TIPO_DESCUENTO,
    MONTO)
  select
    ...
    CANAL_VENTA_ID,
    TIPO_DESCUENTO,
    sum(TIPO_DESCUENTO_MONTO)
  from SE_APRUEBA_GDD.VENTA v join DESCUENTO_VENTA_X_VENTA dv on v.VENTA_ID = dv.VENTA_ID
  join BI_FECHA f on year(v.VENTA_FECHA) = f.FECHA_AÑO and month(v.VENTA_FECHA) = f.FECHA_MES
  join BI_MEDIO_PAGO_VENTA mp on v.VENTA_MEDIO_PAGO = mp.MEDIO_PAGO_ID
  join BI_CANAL_VENTA cv on v.VENTA_CANAL = cv.CANAL_VENTA_ID
  join BI_TIPO_DESCUENTO td on -- TODO
  group by VENTA_ID, FECHA_ID, MEDIO_PAGO_ID, CANAL_VENTA_ID, TIPO_DESCUENTO,
