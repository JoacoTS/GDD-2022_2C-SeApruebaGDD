...
begin
	insert into SE_APRUEBA_GDD.BI_VENTA (FECHA_ID, CLIENTE_ID, RANGO_ID, MEDIO_PAGO_ID, CANAL_VENTA_ID, PROVINCIA_ID, PRODUCTO_ID, CATEGORIA_ID, DESCUENTO_VENTA_ID)
  select
    VENTA_ID,
    FECHA_ID,
    CLIENTE_ID,
    ...
    (select
      PROVINCIA_ID
    from CLIENTE c2 join UBICACION u on c2.CLIENTE_UBICACION = UBICACION_ID
    join CODIGO_POSTAL cp on cp.CODIGO_POSTAL = u.CODIGO_POSTAL
    where c2.CLIENTE_ID = c.CLIENTE_ID)
    PRODUCTO_ID, -- llegamos hasta acá
    CATEGORIA_ID,
    DESCUENTO_VENTA_ID
